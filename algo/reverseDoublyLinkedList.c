...
int main()
{
  ...
  printResult(head);
  reverseList(&head); 
  printDoublyReverseLinkedList(head);
  //  printResult(head);
...
void printDoublyReverseLinkedList(polynomial* head)
{
  if(head)
    {
      //      printf("%d <- ",tail->value);
      printf("(%d x^%d) + ",head->coefficient,head->power);
      printDoublyReverseLinkedList(head -> prev);
    }
  else
    {
      printf("START\n");
