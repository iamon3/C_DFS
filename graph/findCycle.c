...
int isCycleFoundFrom( graphNode* vertex )
{
  ...
  for(temp = vertex -> start ; temp ; temp = temp -> next )
    {
      if ( isCycleFoundFrom( temp -> addressOfThisVertexIs ) ) 
	{
	  printf(" %d <- ",temp -> addressOfThisVertexIs -> vertexNo);
	  return 1;
	}
    }
    
  unVisitAllVertices( head );
  return 0;
}

/*********************************/

void isCycleInGraph( graphNode* vertex )
{
  for( ; vertex ; vertex = vertex -> next)
    {
      if( isCycleFoundFrom( vertex ) )
	{
	  printf(" %d <- start\n\n",vertex -> vertexNo);
	  return ;
	}
...
int main()
{
  ...
  printf("\nLet us find is there any cycle in Graph :\n\n"); 
